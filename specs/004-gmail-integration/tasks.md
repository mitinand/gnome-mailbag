...
- [X] T014 STOP: run `cargo test --workspace` and ./scripts/check.sh; run git diff --check; compare the size with plan.md's table (goa-adapter ~60 production, ~45 fake lines, 5 tests); review constitution I/II; report, suggest the commit, and wait before portion 3.
