...
| Quit | Loads are cancelled and their lines written, then a last line |
...
| A part has a file name | The record says which parameters carry it; never the name. Its shape and extension arrive with attachments |
| The account list is read | One line where the read completes, in `goa-adapter`; no operation identifier |
| An account is named | `account-N` and the provider type, numbered by first appearance; the Online Accounts identifier is never written ([research §5](research.md#5-the-account-label)) |
...
`mailbag::logging` owns the level, the time source, load numbers and account
labels. The library owns the layout. Each crate owns
...
main
  ...
  on quit or window close       cancel loads, then write the quit line

mailbag::logging
  LocalTime                     the formatter's timer, from GLib
  next_load_operation           "load-7"
  account_label                 account-N and the provider type, by first appearance

mailbag::inbox
  begin_load                    keep the load span and start time, info "load started"
  discard_excluded / cancel_load info cancelled with the known reason, before dropping the handle
...
mailbag-content                 debug per selection and decoded part
  header_shape                  raw list headers; only when debug is on
...
crates/mailbag/src/
  ...
  main.rs                       option, start/finish of logging, cancel-before-quit ordering
  inbox.rs                      load span, start time, cancellation and result lines
  window_ui.rs                  span scopes, cancellation on shutdown
...
sites across fifteen existing files, 250–350 lines; a total duration measured
in three places (a load, a read of the account list, opening Settings); two
small functions with logic of their own (`server_text_for_log`,
`header_shape`); no new thread; two small types (level, timer). Tests: about
20 focused cases and marker strings added to existing fixtures. Reassess
before exceeding about 1.5 times this estimate.

`RunningLoad` gains the span and start time already described above. The
...
|---|---|---|
| 1 — The record | Dependencies and regenerated sources; `logging.rs` with the option, both refusals, first and last line, timer and account labels; the checks in `scripts/check.sh`. No events besides start and quit | Escaping, option, label and failing-writer tests; `scripts/check.sh`; Flatpak compilation without build-network access; the native manual runs of [quickstart](quickstart.md). `feat(logging): record a run on request` |
...
| 3 — Load and IMAP | Controller-owned load span and outcomes, cancellation before shutdown, the worker's dispatcher; IMAP step lines, reconnection, part trees with the parameters that carry a file name, certificate errors and server text; the 002 document changes | SC-004–006 on the scripted server; both cancellation reasons, discarded late results, total durations on final lines and sign-in name replacement. `feat(logging): log the Inbox load` |
| 4 — Content, privacy and instructions | Selection and decoding lines, `header_shape`; completed marker fixtures and SC-002/003 over the whole path; README section; installed-Flatpak and independent README acceptance | SC-001–003, SC-007–009; all of [quickstart](quickstart.md), with results outside the repository and unperformed checks left unverified. `feat(logging): log message content decisions and document reporting` |
...
|---|---|
| I — Necessary complexity | One dependency pair chosen against four alternatives, for a need that exists today: a load's lines come from three crates and two threads, and `mailbag-content` may not depend on GLib. The library's formatter is used instead of a layer of our own. No queue or writer thread for the one self-inflicted case of an unread pipe; no file output, rotation, per-component levels, machine-readable format or in-memory history; no shape of file names before anything uses them; no reworking of UI wording for the record; account labels need nothing from the account adapter |
| II — Clear language | The plan states behavior, review points and portions; the line format, fields and 002 wording are in the contract; the comparison of mechanisms is in research. Names say what they do: `server_text_for_log`, `header_shape`, `account_label` |
