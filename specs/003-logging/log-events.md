...
Only the final line of an operation carries a duration
([record contract](contracts/record.md#durations)); a step's duration is the
difference between the times of two lines.
...
| Online Accounts was opened in Settings at the user's request | info | Total duration |
| Settings could not be opened | error | Total duration and `cause`: the name of the launch failure (unavailable, access denied, timeout, invalid reply), the value the UI explains |
...
| What happens | Decision | Fields and notes |
|---|---|---|
| The account list was read | info, in `goa-adapter` | Number of accounts by provider type, total duration |
...
| Reading the account list failed | error, in `goa-adapter`, once per real read | Total duration, the read that failed and `cause`: the name of the failure value (unavailable, access denied, timeout, invalid reply), the value the UI explains. A result published again during Retry is not a new failure and writes nothing |
...
| Account display name, address, icon, Online Accounts identifier | Not logged | Personal details, and an identifier that can hold any text; the label is a number (FR-013, 001 FR-013) |
...
| The load finished and its result was accepted | info | Messages received, of which with content this version does not support (HTML-only, encrypted, S/MIME), total duration. Messages that disappeared are the difference to the rows of "message list loaded" |
...
| The load failed and the failure was accepted | error | Total duration, `step` and `cause` as the names of the failure values the UI explains, the wait limit in seconds for a timeout, the server's response code if any. The reply text and alerts are debug lines of `mailbag-imap` |
| Every message of the window disappeared, or the Inbox was replaced | error | Total duration, cause "Inbox changed"; only while the result remains applicable |
| The load was cancelled by exclusion of its account or by quitting | info, in `discard_excluded` or `cancel_load` | Reason and elapsed load duration, before dropping an active handle; no waiting for the worker |
| A result arrived for an account excluded meanwhile and was dropped | info, at the controller's result applicability check | Label; no completion, warning or error for the discarded result |
...
| The mail worker stopped without a result | error | Total duration, cause "worker stopped"; only while the result remains applicable |
...
| Text was loaded | info | Messages, commands sent, bytes received |
| One command's group of messages completed or failed | debug | Sections requested, UIDs, bytes received, and that it failed when it did |
...
| A part was decoded | debug | Folder, UID, section, character set, transfer encoding, whether `format=flowed` was applied, bytes in, characters out |
