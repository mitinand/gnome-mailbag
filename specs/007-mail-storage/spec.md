...
**Created**: 2026-09-25
**Status**: Draft, challenged 2026-09-25. The decisions taken at sizing and at
the specification challenge are recorded under Clarifications.
...
- **FR-013 — The store's failures after the start**: A failure after the start
  is the failure of the operation that met it, declared under 006 in the code
  that owns it (006 FR-001, FR-012). A result that cannot be written makes the
  load fail (FR-004), shown like any failed load. A list that cannot be read
  for the selected account leaves nothing to show, so the failure page takes
  the list's place, as for a failed load. Panics and cancellation follow 006
  FR-014 and FR-010.
...
- **FR-014 — Deferred, with the layer each waits for**:
  ...
  load's completion; from then on a store is never discarded without the
  user's consent to losing unsent changes.
...
- **SC-001**: After a completed load of each provider's scripted server and a
  ...
  same text or status page, and the server records zero connection attempts
  (US1; FR-001, FR-002).
...
- **SC-003**: A load that fails at each step, a cancelled load and a load whose
  result cannot be written each leave the stored rows exactly as before; with
  rows the banner is shown, without rows the failure page (US3, US5; FR-004,
  FR-005, FR-013).
...
- The size budget approved at sizing: at most 600 net new production lines,
