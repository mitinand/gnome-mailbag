...
/// The debug lines one call writes.
fn record_of(call: impl FnOnce()) -> String {
    let record = crate::test_record::CapturedRecord::start(tracing::Level::DEBUG);
    call();
    record.text()
}

#[test]
fn each_selection_decision_has_a_line_where_it_is_made() {
    let alternative = record_of(|| {
        select_text_parts(&load_sample("04-alternative.eml").root);
    });
    assert!(
        alternative.lines().any(|line| line
            .contains("the last alternative with plain text was chosen")
            && line.contains("alternative=1")),
        "{alternative}"
    );
    assert!(
        alternative.contains(r#"text parts selected sections="1""#),
        "{alternative}"
    );
    let named_root = record_of(|| {
        select_text_parts(&load_sample("20-related-start.eml").root);
    });
    assert!(
        named_root
            .lines()
            .any(|line| line.contains("the root of a related set was chosen")
                && line.contains("start_matched=true")),
        "{named_root}"
    );
    let first_root = record_of(|| {
        select_text_parts(&load_sample("14-related.eml").root);
    });
    assert!(first_root.contains("start_matched=false"), "{first_root}");
    let html_only = record_of(|| {
        select_text_parts(&load_sample("05-html-only.eml").root);
    });
    assert!(
        html_only.contains("no text part selected explanation=NoPlainText"),
        "{html_only}"
    );
}

#[test]
fn a_decoded_part_is_described_without_its_text() {
    let sample = load_sample("02-cp1251-base64.eml");
    let (header, body) = &sample.sections["1"];
    let mut text = String::new();
    let record = record_of(|| text = decode_text_part(header, body).expect("decodes"));
    assert!(
        record.contains(r#"charset="windows-1251" transfer_encoding="base64" flowed=false"#),
        "{record}"
    );
    assert!(
        record.contains(&format!("characters_out={}", text.chars().count())),
        "{record}"
    );
    let first_word = text.split_whitespace().next().expect("text");
    assert!(!record.contains(first_word), "{record}");
    let flowed = load_sample("18-flowed.eml");
    let (header, body) = &flowed.sections["1"];
    let record = record_of(|| {
        decode_text_part(header, body).expect("decodes");
    });
    assert!(record.contains("flowed=true"), "{record}");
}

#[test]
fn a_list_header_that_does_not_decode_is_named_without_its_value() {
    // To is absent, which is normal and writes nothing.
    let header_lines = b"Subject: \xff\xfe marker-header-value\r\nFrom:\r\n";
    let record = record_of(|| {
        decode_display_fields(header_lines);
    });
    assert_eq!(
        record.lines().count(),
        2,
        "one line per undecoded header:\n{record}"
    );
    for expected in [
        r#"list header has replacement characters header="Subject""#,
        r#"list header gave no value header="From""#,
    ] {
        assert!(
    ...
    assert!(!record.contains("marker-header-value"), "{record}");
    let complete = record_of(|| {
        decode_display_fields(
            b"Subject: Hello\r\nFrom: a@example.invalid\r\nTo: b@example.invalid\r\n",
        );
    });
    assert!(
        complete.is_empty(),
        "a complete row writes nothing:\n{complete}"
    );
}
