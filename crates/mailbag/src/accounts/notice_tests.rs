...
#[test]
fn unapplied_switches_do_not_clear_selection_or_produce_notices() {
    let mut accounts = AccountList::default();
    let enabled = make_checked_list(&[("one", make_account_details(AccountProvider::Google))]);
    accounts.apply_update(enabled.clone());
    accounts.select_account(Some(make_account_id("one")));
    // Model what the UI receives after the adapter replaces a pending
    // disabled/absent update with the latest enabled account list.
...
fn explicit_disable_wins_over_unrelated_errors_and_reappearance_does_not_select() {
    ...
    assert_eq!(
        accounts.apply_update(disabled.clone()),
